...
import { getNewsletterTranslation } from '../NewsletterForm/translations';
import Link from 'next/link';

type NewsletterInfoVariant = 'success' | 'warning' | 'info';

interface NewsletterInfoProps {
  variant?: NewsletterInfoVariant;
  title?: string;
  message: string;
  topicName?: string;
  email?: string;
...
export function NewsletterInfo({
  variant = 'success',
  title,
  message,
  topicName,
      ...
      <div className="flex-1 max-w-2xl mx-auto px-4 py-12">
        <div className="bg-white rounded-lg border border-gray-200 p-8">
          <div className="text-center">
            ...
            {(topicName || email) && (
              <div className="bg-[#f8f7f7] rounded-lg p-6 mb-6 text-left">
                <h3 className={`text-lg font-semibold text-gray-900 mb-4 ${garamond.className}`}>
                  {locale === 'pl' ? 'Informacje o subskrypcji' : 'Subscription Information'}
                </h3>
                <dl className="space-y-3">
                  {email && (
                    <div>
                      <dt className="text-sm font-medium text-gray-500">
                        {locale === 'pl' ? 'Adres email' : 'Email address'}
                      </dt>
                  ...
                  {topicName && (
                    <div>
                      <dt className="text-sm font-medium text-gray-500">
                        {locale === 'pl' ? 'Temat subskrypcji' : 'Subscription Topic'}
                      </dt>
