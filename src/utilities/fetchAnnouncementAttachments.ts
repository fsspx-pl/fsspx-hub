...
/**
 * Extracts and fetches media attachments from an announcement's lexical content.
 * Also returns the attachment display settings with defaults.
 */
export async function fetchAnnouncementAttachments(announcement: Announcement): Promise<AttachmentResult> {
  const mediaIds = announcement.content ? extractMediaFromLexical(announcement.content) : []
  ...
  const attachmentDisplay = announcement.attachmentDisplay || {
...
export async function prepareEmailAttachments(
  ...
  const results = await Promise.all(
    mediaList.map(async (media) => {
      try {
        return await getMediaAsEmailAttachment(media, announcementId)
      } catch (error: any) {
